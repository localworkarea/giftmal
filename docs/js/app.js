(() => {
    ...
    let _slideDown = (target, duration = 500, showmore = 0, maxHeight = 0) => {
        if (!target.classList.contains("_slide")) {
            target.classList.add("_slide");
            ...
            target.style.marginBottom = 0;
            target.offsetHeight;
            if (maxHeight > 0) height = Math.max(100, Math.min(height, maxHeight));
            target.style.transitionProperty = "height, margin, padding";
            target.style.transitionDuration = duration + "ms";
            ...
            target.style.removeProperty("margin-bottom");
            window.setTimeout(() => {
                if (maxHeight === 0) target.style.removeProperty("height");
    ...
    let _slideToggle = (target, duration = 500, showmore = 0, maxHeight = 0) => {
        if (target.hidden || target.style.height === "0px") return _slideDown(target, duration, showmore, maxHeight); else return _slideUp(target, duration, showmore);
    };
    ...
    function tabs() {
        ...
        document.addEventListener("click", function(e) {
            ...
            if (link) {
                ...
                if (hash && hash.startsWith("#tab-")) {
                    ...
                    if (tabsBlock) setActiveTab(tabsBlock, Number(tabsActiveHash[1]), true);
                }
            }
        });
        ...
        function setActiveTab(tabsBlock, tabIndex, updateHash = false) {
            ...
            if (activeTitle && activeContent) {
                ...
                if (updateHash) setHash(`tab-${tabsBlock.dataset.tabsIndex}-${tabIndex}`);
            }
        }
        ...
        videoContainers.forEach((container, index) => {
            ...
            if (playButton && previewElement && playerElement && videoUrl) {
                ...
                playButton.addEventListener("click", event => {
                    ...
                    } else console.error("Failed to create iframe");
                });
    ...
    function initLiveChat() {
        ...
        document.addEventListener("click", event => {
            if (!liveChat.contains(event.target) && chatButton.getAttribute("aria-expanded") === "true") toggleModal();
        });
    ...
    function initCopyLinks(root = document) {
    ...
    function initSliders() {
        ...
        blogSliders.forEach(slider => {
            if (document.querySelector(slider.selector)) new Swiper(slider.selector, {
    ...
    let _slideToggleSelect = (target, duration = 500, maxHeight = 0) => {
        ...
        if (parseFloat(computedStyle.opacity) === 0 || target.style.height === "0px") return _slideDownSelect(target, duration, maxHeight); else return _slideUpSelect(target, duration);
    };
    ...
    class ScrollWatcher {
        ...
        scrollWatcherCallback(entry, observer) {
            ...
            if (targetElement.dataset.watch === "orders") {
                if (entry.isIntersecting) this.visibleOrders.add(targetElement); else this.visibleOrders.delete(targetElement);
                if (this.visibleOrders.size > 0) document.documentElement.classList.add("_watch-orders"); else document.documentElement.classList.remove("_watch-orders");
            }
        ...
        function pageNavigationAction(e) {
            if (e.type === "click") {
                ...
                if (targetElement.closest("[data-goto]")) {
                    ...
                    if (flsModules.fullpage) ; else gotoBlock(gotoLinkSelector, noHeader, gotoSpeed, offsetTop);
                    e.preventDefault();
                }
    ...
    function handleNotification({notificationSelector, buttonSelector, storageKey, onHide = function() {}, additionalElements = {}, useCookies = false}) {
        ...
        const isClosed = useCookies ? getCookie(storageKey) === "true" : getLocalStorage(storageKey) === "true";
        if (isClosed) {
        ...
        function hideNotification() {
            ...
            setTimeout(function() {
                notification.style.display = "none";
                onHide();
                if (useCookies) setCookie(storageKey, "true"); else setLocalStorage(storageKey, "true");
            }, 300);
        }
    ...
    function initMobileAppAd() {
        if (localStorage.getItem(APP_AD_SHOWN_KEY) === "true") return;
        ...
        if (!adNotification || !adCloseButton) return;
        setTimeout(() => {
        ...
        if (adButton) adButton.addEventListener("click", () => {
    ...
    document.addEventListener("DOMContentLoaded", () => {
        ...
        window.matchMedia("(min-width: 56.311em)");
        const mediaQuery900max = window.matchMedia("(max-width: 56.311em)");
        ...
        checkoutBodies.forEach(body => {
            ...
            if (parent) if (body.scrollHeight > body.clientHeight) body.classList.add("_more-content"); else body.classList.remove("_more-content");
        });
        ...
        filterSections.forEach(section => {
            ...
            function handleMediaChange(e) {
                if (e.matches) if (e.media === mediaQuery480max.media) {
                    if (filterSpollers) _slideUp(filterSpollers, 0);
                } else if (e.media === mediaQuery480min.media) if (filterSpollers) filterSpollers.hidden = false;
            }
            handleMediaChange(mediaQuery480max);
        ...
        function adjustWrapperMaxHeight(wrapper) {
            ...
            if (body) {
                if (spaceBelow < 250) availableHeight = Math.max(110, availableHeight);
                ...
                for (let element of childElements) if (element.scrollHeight > availableHeight) {
                    isContentOverflowing = true;
                    break;
                }
                if (isContentOverflowing) body.classList.add("_more-content"); else body.classList.remove("_more-content");
            }
        }
        ...
        document.querySelectorAll(".popup-rules-item").forEach(popupRules => {
            ...
            function openPopupFrom(element) {
                ...
                if (popupSelector) flsModules.popup.open(popupSelector);
            }
            ...
            function handleAddButtonClick(event) {
                ...
                if (isConfirmed && popupSelector) flsModules.popup.open(popupSelector); else if (mediaQuery480max.matches && !isConfirmed) {
            ...
            if (confirmButton) confirmButton.addEventListener("click", function() {
                ...
                setTimeout(() => {
                    ...
                    if (popupSelector) flsModules.popup.close(popupSelector);
                }, 100);
            });
        ...
        function handleCardItemClick(event) {
            if (mediaQuery480max.matches) if (typeof flsModules !== "undefined" && flsModules.popup && typeof flsModules.popup.close === "function") setTimeout(() => {
                flsModules.popup.close();
            }, 100);
        }
        ...
        intlTelInputs.forEach(input => {
            ...
            function attachSearchInputListener(itiInstance) {
                setTimeout(() => {
                    ...
                    if (searchInput) searchInput.addEventListener("input", () => updateNoResultsMessage(searchInput, dropdownContent));
                }, 100);
            }
    ...
    function initMobilePickers() {
        ...
        document.addEventListener("beforePopupClose", event => {
            if (event.detail.popup.targetOpen.selector === "#popupRolldate") rdInstances.forEach(inst => inst?.hide?.());
        });
    }
    function toggleDatepicker(e) {
        if (e.matches) initMobilePickers(); else initDesktopPickers();
    }
