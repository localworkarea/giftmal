...
function initPostTablesScroll() {
  ...
  const hasOverflowX = (el) =>
    el.scrollWidth > el.clientWidth + EPS;

  const isScrollEnd = (el) =>
...
function initPostToc(options = {}) {
  ...
  const initial =
    ...
    items.some((i) => i.id === window.location.hash)
      ? window.location.hash
      : items[0].id;
